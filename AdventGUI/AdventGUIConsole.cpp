...
bool AdventGUIConsole::ParseFormatTokens(const char* str, std::vector<FormatToken>& outTokens) const
{
	assert(strlen(str) != 0);
	...
	{ 
		...
		if (*currentChar == '<')
		{
			...
			const char* startTag = currentChar;
			while (isalpha(*currentChar)) { ++currentChar; }

			strncpy_s(newToken.Name, FormatToken::MAX_TOKEN_LENGTH, startTag, currentChar - startTag);
			...
			while (*currentChar != '\0')
			{
				...
				{
					const char* attributeStart = currentChar;
					do 
					{
						++currentChar;
...
const char* AdventGUIConsole::GetNextToken(const char* str, char* outBuffer, size_t outBufferSize) const
{
	const char* currentChar = str;
	while (!isalpha(*currentChar) && *currentChar != '\0')
	{
		++currentChar;
	}
